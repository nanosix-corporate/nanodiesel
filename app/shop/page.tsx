        ...
        <div className="max-w-[1200px] mx-auto text-left relative z-10">
          <div className="max-w-4xl">
            ...
            <p className="text-base md:text-lg text-olive-300 leading-relaxed max-w-2xl">
              Nano Diesel tersedia dalam berbagai kemasan untuk kendaraan pribadi, armada, hingga industri berskala besar.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {products.map((product) => (
              ...
              >
                {product.badge && (
                  <div className={`absolute top-3 right-3 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider z-10 shadow-lg ${product.badge === 'Best Seller' ? 'bg-emerald-600' :
                ...
                <div className="p-3 flex-grow flex flex-col gap-1">
                  <h3 className={`text-xs md:text-sm font-bold font-headline leading-tight line-clamp-2 ${product.dark ? 'text-white' : 'text-brand-dark'}`}>
                    {product.title}
                  </h3>
                  <span className={`text-xs font-medium ${product.dark ? 'text-olive-400/70' : 'text-olive-400'}`}>
                    {product.subtitle}
                  </span>
                  <p className={`text-xs leading-relaxed line-clamp-1 ${product.dark ? 'text-olive-400' : 'text-brand-copy/80'}`}>
                    {product.description}
                  </p>
                  {!product.dark && (
                    <a
                      href={`/shop/${product.id.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '')}`}
          ...
          <div className="mt-16 md:mt-20 rounded-2xl bg-gradient-to-br from-brand-dark via-brand-dark to-brand-darkest border border-emerald-900/20 p-8 md:p-12 text-center text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-2xl md:text-4xl font-headline font-black mb-4">Tersedia di Marketplace Resmi</h2>
              <p className="text-olive-300 mb-8 max-w-lg mx-auto leading-relaxed">
                Dapatkan Nano Diesel dengan mudah
                <br />
                <strong>melalui Shopee dan Tokopedia Official Store kami.</strong>
              </p>
