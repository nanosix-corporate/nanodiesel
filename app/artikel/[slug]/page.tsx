    ...
    types: {
      productCard: ({ value }: any) => {
        ...
        if (productInfo) {
          ...
          const legacyPlatform = value.linkUrl.includes('shopee')
            ? 'shopee'
            : value.linkUrl.includes('tokopedia')
            ? 'tokopedia'
            : undefined;
          buttonsToRender.push({
            url: legacyPlatform
            ...
            <table className="w-full text-sm text-left">
                ...
                <thead className="bg-emerald-900 text-white">
                  <tr>
                    {(headers as string[]).map((header, i) => (
                      <th key={i} scope="col" className="px-4 py-3 font-semibold text-xs uppercase tracking-wider whitespace-nowrap">
                        {header}
                      </th>
                    ))}
                  </tr>
              ...
              {hasRows && (
                <tbody className="divide-y divide-olive-100">
                  {(rows as any[]).map((row, rowIdx) => (
                    <tr key={rowIdx} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-olive-50'}>
                      {Array.isArray(row.cells) && (row.cells as string[]).map((cell, cellIdx) => (
                        <td key={cellIdx} className="px-4 py-3 text-brand-dark align-top">
                          {cell}
                        </td>
                      ))}
                    </tr>
        ...
        return (
          <div className="my-10 not-prose">
            ...
            <div className="divide-y divide-olive-200 border border-olive-200 rounded-xl overflow-hidden">
              {(value.items as any[]).map((item, idx) => (
                <details key={idx} name={`faq-article-${value._key || 'block'}`} className="group bg-white">
